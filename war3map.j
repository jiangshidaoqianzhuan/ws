...
function CreateBuildingsForPlayer0 takes nothing returns nothing
    ...
    local trigger t
    local real life
    set u=CreateUnit(p, 'nwc1', - 448.0, - 1344.0, 270.000)
    set u=CreateUnit(p, 'n015', - 192.0, - 1344.0, 270.000)
endfunction
//===========================================================================
...
function Trig______________003Actions takes nothing returns nothing
    ...
    call SaveInteger(YDHT, GetHandleId(GetTriggeringTrigger()), 0xCFDE6C76, ydl_localvar_step)
    call SaveInteger(YDHT, GetHandleId(GetTriggeringTrigger()), 0xECE825E7, ydl_localvar_step)
    call DisplayTextToForce(GetPlayersAll(), ( ( YDWEGetPlayerColorString(GetOwningPlayer(GetDyingUnit()) , GetUnitName(GetDyingUnit())) + ( "被" + GetPlayerName(GetOwningPlayer(GetKillingUnitBJ())) ) ) + "|CFFFF0000杀死了！|R" ))
...
function Trig____________________002Actions takes nothing returns nothing
    if ( ( GetUnitTypeId(GetAttackedUnitBJ()) == 'n014' ) and ( ( GetUnitTypeId(GetAttacker()) == 'n00S' ) or ( GetUnitTypeId(GetAttacker()) == 'n00O' ) or ( GetUnitTypeId(GetAttacker()) == 'n00Z' ) or ( GetUnitTypeId(GetAttacker()) == 'n011' ) or ( GetUnitTypeId(GetAttacker()) == 'n00X' ) or ( GetUnitTypeId(GetAttacker()) == 'n013' ) or ( GetUnitTypeId(GetAttacker()) == 'n012' ) or ( GetUnitTypeId(GetAttacker()) == 'n00W' ) or ( GetUnitTypeId(GetAttacker()) == 'n010' ) or ( GetUnitTypeId(GetAttacker()) == 'n00P' ) ) ) then
        call ShowUnit(GetAttacker(), false)
    else
    endif
endfunction
//===========================================================================
function InitTrig____________________002 takes nothing returns nothing
    ...
    call YDWESyStemAnyUnitDamagedRegistTrigger(gg_trg____________________002)
    call TriggerAddAction(gg_trg____________________002, function Trig____________________002Actions)
endfunction
//===========================================================================
// Trigger: 靠近动物增加仇恨 001
//===========================================================================
function Trig__________________________001Actions takes nothing returns nothing
    local group ydl_group
    local unit ydl_unit
    local integer ydl_localvar_step= LoadInteger(YDHT, GetHandleId(GetTriggeringTrigger()), 0xCFDE6C76)
    set ydl_localvar_step=ydl_localvar_step + 3
    ...
    loop
        exitwhen bj_forLoopAIndex > bj_forLoopAIndexEnd
        call SaveReal(YDHT, GetHandleId(GetTriggeringTrigger()) * ydl_localvar_step, 0xC4E1B9D4, 0.00)
        if ( ( udg_Dw[bj_forLoopAIndex] != null ) and ( IsUnitAliveBJ(udg_Dw[bj_forLoopAIndex]) == true ) ) then
            set ydl_group=CreateGroup()
            call GroupEnumUnitsInRange(ydl_group, GetUnitX(udg_Dw[bj_forLoopAIndex]), GetUnitY(udg_Dw[bj_forLoopAIndex]), 400.00, null)
            loop
                set ydl_unit=FirstOfGroup(ydl_group)
                exitwhen ydl_unit == null
                call GroupRemoveUnit(ydl_group, ydl_unit)
                // 此处条件可能有问题，因为我不知道怎么去判断是不是动物
                // 只要是动物，条件即成立，反之不成立。
                // 现在的条件是判断是不是中立被动
                if ( ( GetOwningPlayer(ydl_unit) == Player(PLAYER_NEUTRAL_PASSIVE) ) ) then
                    if ( ( LoadReal(YDHT, GetHandleId(GetTriggeringTrigger()) * ydl_localvar_step, 0xC4E1B9D4) < ( ( 400.00 - YDWEDistanceBetweenUnits(ydl_unit , udg_Dw[bj_forLoopAIndex]) ) * 2.50 ) ) ) then
                        call SaveReal(YDHT, GetHandleId(GetTriggeringTrigger()) * ydl_localvar_step, 0xC4E1B9D4, ( ( 400.00 - YDWEDistanceBetweenUnits(ydl_unit , udg_Dw[bj_forLoopAIndex]) ) * 2.50 ))
                    else
                    endif
                else
                endif
            ...
            if ( ( LoadReal(YDHT, GetHandleId(GetTriggeringTrigger()) * ydl_localvar_step, 0xC4E1B9D4) == 0.00 ) ) then
                if ( ( GetPlayerState(ConvertedPlayer(bj_forLoopAIndex), PLAYER_STATE_RESOURCE_FOOD_USED) > 0 ) ) then
                    call AdjustPlayerStateBJ(- 1, ConvertedPlayer(bj_forLoopAIndex), PLAYER_STATE_RESOURCE_FOOD_USED)
                else
                    set udg_xiaochouhen[bj_forLoopAIndex]=0.00
                endif
            else
                if ( ( GetPlayerState(ConvertedPlayer(bj_forLoopAIndex), PLAYER_STATE_RESOURCE_FOOD_USED) < 100 ) ) then
                    set udg_xiaochouhen[bj_forLoopAIndex]=( udg_xiaochouhen[bj_forLoopAIndex] + LoadReal(YDHT, GetHandleId(GetTriggeringTrigger()) * ydl_localvar_step, 0xC4E1B9D4) )
                    call AdjustPlayerStateBJ(R2I(( udg_xiaochouhen[bj_forLoopAIndex] / 100.00 )), ConvertedPlayer(bj_forLoopAIndex), PLAYER_STATE_RESOURCE_FOOD_USED)
                    set udg_xiaochouhen[bj_forLoopAIndex]=ModuloReal(udg_xiaochouhen[bj_forLoopAIndex], 100.00)
                else
                endif
                if ( ( GetPlayerState(ConvertedPlayer(bj_forLoopAIndex), PLAYER_STATE_RESOURCE_FOOD_USED) > 100 ) ) then
                    call SetPlayerStateBJ(ConvertedPlayer(bj_forLoopAIndex), PLAYER_STATE_RESOURCE_FOOD_USED, 100)
                else
                endif
            endif
        else
...
//===========================================================================
function InitTrig__________________________001 takes nothing returns nothing
    set gg_trg__________________________001=CreateTrigger()
    call TriggerRegisterTimerEventPeriodic(gg_trg__________________________001, 0.50)
...
call ExecuteFunc("jasshelper__initstructs12720196")
...
function jasshelper__initstructs12720196 takes nothing returns nothing
